...
class SBDImplementation(Plugin_skeleton):

    def query(self, dbg):
        if platform.linux_distribution()[1] == '7.5.0':
            return {
                "plugin": "sbd+raw+qdisk",
                "name": "The Sheepdog QEMU qdisk user-space datapath plugin",
                "description": ("This plugin manages and configures qdisk"
                                " instances backend for Sheepdog VDI in raw format built"
                                " using libsbd"),
...
if __name__ == "__main__":
    log.log_call_argv()
    CMD = Plugin_commandline(SBDImplementation())
